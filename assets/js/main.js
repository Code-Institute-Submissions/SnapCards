...
class SnapCardGame {
	...
	cardMatchCheck(card) {
		...
		if (this.cardtype(card) === this.cardtype(this.cardCheck)) this.cardMatch(card, this.cardCheck);
		else this.cardMisMatch(card, this.cardCheck);
		this.cardCheck = null;
	}
	...
	cardMatch(card1, card2) {
		...
		if (this.matchedCards.length === this.cardsArray.length) this.winning();
	}
	// cards do not match
